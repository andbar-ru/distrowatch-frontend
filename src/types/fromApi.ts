import { Array as A, Static, Record, String as S, Number as N } from "runtypes";

const PositiveIntNumber = N.withConstraint(
  (n) => (Number.isInteger(n) && n > 0) || `${n} is not positive integer`
);
const DateString = S.withConstraint(
  (s) => !isNaN(new Date(s).valueOf()) || `${s} is not a date or invalid date`
);
...
export const DistrRecords = A(DistrRecord);
